...
//    }
    ...
    static String getJavaBasicTypeByAsm(String asmType) {
        if (null == asmType) {
            return null
        }
    ...
    static String cleanAsmMethodDesc(String desc) {
...
//        Log.i("PluginUtil", "desc after clean:" + String.format("(%s)", sb.toString()))
    ...
    static boolean isNeedScanClassFile(File file) {
        if (null == file) return false

        if (!file.isFile()) return false

        if (!file.absolutePath.endsWith(".class")) return false

        return true
    }

    static boolean isPathNeedScan(String path) {
        if (null == path || path.isEmpty()) return false
        return true
    }

    static boolean isGeneratedClass(String path) {
    ...
    static boolean isJarNeedScan(String name) {
    ...
    static void dumpJsonFile(Object obj, String fileName) {
